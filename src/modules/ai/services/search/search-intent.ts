import { containsSearchPhrase, normalizeSearchText } from '../../../../common/search/search-text';
...
export function evaluateProduct(product: SearchableProduct, intent: SearchIntent) {
  const title = normalizeSearchText(product.title);
  const keywords = normalizeSearchText((product.keywords ?? []).join(' '));
  const description = normalizeSearchText(product.description ?? '');
  const matches = (text: string, term: string) => containsSearchPhrase(text, term)
    || (!intent.categoryCode && intent.terms.length === 1 && term.length >= 3
  ...
  const coverage = (text: string) => intent.terms.filter(term => matches(text, term)).length / intent.terms.length;
  ...
  const identityCategory = product.categoryCode
    ?? namedCategory(title, intent)
    ?? namedCategory(keywords, intent);
  const categoryInIdentity = intent.categoryCode !== undefined
  ...
  const categoryInDescription = intent.aliases.some(alias => {
    ...
    return (identityCategory === undefined && start.split(' ').slice(0, 3).includes(alias))
      || intent.identityPrefixes.some(prefix => intent.articles.some(article =>
  ...
  const primary = intent.categoryCode
    ...
    : titleCoverage >= 0.5 || keywordCoverage >= 0.5 || descriptionCoverage === 1;
  const relatedType = !intent.categoryCode || (identityCategory !== undefined
    && intent.relatedCategoryCodes.includes(identityCategory));

  let score = titleCoverage * 0.65 + keywordCoverage * 0.2 + descriptionCoverage * 0.15;
  if (intent.terms.length && intent.terms.every(term => matches(`${identity} ${description}`, term))) score += 0.2;
