...
import { normalizeSearchText, searchTerms } from '../../common/search/search-text';
...
export class CategoryTaxonomyService {
  private readonly aliasesByLocale = new Map<string, { expiresAt: number; data: CategoryAlias[] }>();
  private readonly relatedByCategory = new Map<string, { expiresAt: number; data: string[] }>();
  ...
  async createSearchIntent(query: string, locale = 'es-BO'): Promise<SearchIntent> {
    ...
    const matches = aliases
      ...
      .sort((a, b) => text.indexOf(a.normalizedAlias) - text.indexOf(b.normalizedAlias)
    ...
    const categoryAliases = categoryCode
      ? aliases.filter(alias => alias.category.code === categoryCode).map(alias => alias.normalizedAlias)
      : [];
  ...
  private async loadAliases(locale: string): Promise<CategoryAlias[]> {
    ...
    const data = await this.aliasRepository
      ...
      .getMany();
    this.aliasesByLocale.set(normalizedLocale, { expiresAt: Date.now() + this.cacheTtlMs, data });
    return data;
  }

  private async loadRelatedCategoryCodes(categoryCode: string): Promise<string[]> {
    const cached = this.relatedByCategory.get(categoryCode);
    if (cached && cached.expiresAt > Date.now()) return cached.data;
    const relations = await this.relationRepository
      .createQueryBuilder('relation')
      .innerJoin('relation.sourceCategory', 'source', 'source.code = :categoryCode', { categoryCode })
    ...
    this.relatedByCategory.set(categoryCode, { expiresAt: Date.now() + this.cacheTtlMs, data });
    return data;
  }
}
